"""Tests for Coach Chat: mock llm_client.generate(), send message, verify chat history and Clear button."""
...
def _find_text_input(app: AppTest, key: str):
    """Return the first text_input widget whose key matches."""
    for inp in app.text_input:
        if inp.key == key:
            return inp
    raise AssertionError(f"No text_input with key={key!r} found")


# ---------------------------------------------------------------------------
# Coach Chat — Dashboard (compact) section
# ---------------------------------------------------------------------------


class TestDashboardCoachSendMessage:
    """Send a message via the dashboard coach chat section."""

    def test_send_empty_input_does_nothing(self, app):
        with patch("src.agent.llm_client.generate", return_value="Response") as mock_generate:
            _go(app, "Dashboard")
            _find_text_input(app, "dash_coach_input").set_value("   ").run()
            _find_button(app, "dash_coach_send").click().run()
            mock_generate.assert_not_called()
        assert len(app.session_state.coach_messages) == 0


class TestDashboardCoachClear:
    """Clear button empties coach chat history."""

    def test_clear_button_empties_messages(self, app):
        """Clicking Clear removes all coach messages."""
        _go(app, "Dashboard")
        app.session_state.coach_messages = [
            {"role": "user", "content": "Hello"},
            {"role": "assistant", "content": "Hi there"},
        ]
        app.run()

        _find_button(app, "dash_coach_clear").click().run()

        assert app.session_state.coach_messages == []
