...
def compute_features(
    ...
    Args:
        ...
        morning_checkins: List of dicts with keys: date, perceived_readiness,
            soreness, stress, sleep_quality, mood, energy, motivation,
            pain_score
    ...
    if activity_metrics:
        ...
        if not act_df.empty and "start_date" in act_df.columns:
            ...
            daily_activity["date"] = pd.to_datetime(daily_activity["start_date"])
    ...
    if "soreness" in df.columns and "stress" in df.columns:
        soreness_norm = df["soreness"].fillna(3.5) / 7.0
        stress_norm = df["stress"].fillna(3.5) / 7.0
